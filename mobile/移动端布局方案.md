...
一个物理像素是显示器(手机屏幕)上最小的物理显示单元，我们操作css像素，然后由物理像素进行表达，在Retina屏出来以前，css像素的数量 === 物理像素。
...
#### 引用
在所有资源加载之前执行这个js。
执行这个js后，会在html（也就是document.documentElement）上增加一个data-dpr属性，以及font-size样式。之后页面中的元素，都可以用rem单位来设置。html上的font-size就是rem的基准像素。
...
#### 把视觉稿中的px转换成rem
以750的视觉稿为例，把视觉稿分成100份来看待（为了以后兼容vh，vw单位）。每一份被称为一个单位a。同时，1rem单位认定为10a。
