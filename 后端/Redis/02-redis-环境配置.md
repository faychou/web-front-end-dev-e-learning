...
``` bash
# Redis 是 C 语言开发，需要先将源码进行编译，编译依赖 gcc 环境
yum -y install gcc automake autoconf libtool make

# 下载 Redis
wget http://download.redis.io/releases/redis-5.0.0.tar.gz
#或者通过 sftp 上传安装包到 linux
...
# 创建并运行容器
docker run --rm -d --name redis6379 -p 6379:6379 redis:5.0.0 --requirepass "fay"

# 测试Redis
docker exec -it redis6379 bash    # 进入 redis 命令 
...
Redis 自带了基于命令行的 Redis 客户端，即 redis-cli：
...
Redis 定义了很多默认配置。但在实际开发中，一般我们都会通过手动配置完成。

Redis 的配置文件位于 Redis 安装目录下，文件名为 `redis.conf`。

将解压后 Redis 目录下的 `redis.conf ` 文件复制到指定的安装目录：
